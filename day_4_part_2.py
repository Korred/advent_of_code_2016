room_list = '''nzydfxpc-rclop-qwzhpc-qtylyntyr-769[oshgk]
...
rwcnawjcrxwju-ljwmh-bqryyrwp-277[nxatm]'''.split("\n")
...
def rotate(c,rot):
  ...
  while left != 0:
    if curr + left <= 122:
      ...
      left = left - (122-curr) - 1
      curr = 97
  return chr(curr + left)
  

sum_of_real_ids = 0
real_rooms = []
for room in room_list:
  name = re.search(r'[^\d]+-', room).group().replace("-","")
  name_parts = re.search(r'[^\d]+-', room).group()[:-1].split("-")
  room_id = int(re.search(r'([\d]+)', room).group())
  checksum = re.search(r'\[(.*)\]', room).group(1)
  
  top_five = sorted(list(Counter(name).items()), key = lambda x: (-x[1],x[0]))[:5]
  top_checksum = "".join(i[0] for i in top_five)
  if top_checksum == checksum:
    real_rooms.append((name_parts, room_id))
...
for room in real_rooms:
  ...
  for part in room[0]:
    decoded_part = ""
    for char in part:
      decoded_part += rotate(char, room[1])
    decoded_name.append(decoded_part)
  joined = " ".join(decoded_name)
  if "north" in joined:
    print(joined, room[1])
