...
def has_abba(seq_list):
  for s in seq_list:
    ngrams = list(zip(*[s[i:] for i in range(4)]))
    for g in ngrams:
      if g[0] == g[3] and g[1] == g[2] and g[0] != g[1]:
        return True
  return False
...
for ip in ip_list:
  ...
  if has_abba(ip_seq) and not has_abba(hypernet_seq):
    ip_tls_count += 1

print("IPs with TLS: ", ip_tls_count)
