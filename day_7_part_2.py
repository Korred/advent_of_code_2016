...
def get_ip_seq(ip,hyper):
  ip_seq = ip
  for s in hyper:
    ip_seq = ip_seq.replace(s," ")
  return ip_seq.split(" ")
  
def supports_ssl(ips,hyps):
  candidates = []
  for s in ips:
    ngrams = list(zip(*[s[i:] for i in range(3)]))
    for g in ngrams:
      if g[0] == g[2] and g[0] != g[1]:
        candidates.append(g)
...
ip_ssl_count = 0  
for ip in ip_list:
  hypernet_seq = re.findall(r'\[[\w]+\]',ip)
  ip_seq = get_ip_seq(ip, hypernet_seq)
  hypernet_seq = [i[1:-1] for i in hypernet_seq] # removing [ and ] from entries
  
  if supports_ssl(ip_seq, hypernet_seq):
    ip_ssl_count += 1

print("IPs with SSL: ", ip_ssl_count)
